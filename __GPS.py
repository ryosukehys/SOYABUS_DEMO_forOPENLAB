...
def getGPS(comport_name, dataNum, rate):
    ...
    while (GPRMC == -1):
        ...
        tmp_string ="$GPRMC,022313.00,A,4305.7218591,N,14120.4855501,E,0.0,,130723,9.7,W,D*0A"
            ...
            try:
                try:
                    if (float(tmp2[GPRMC+1]) >= 150000):
                        time_jst = float(tmp2[GPRMC+1]) - 150000 # UTC time is transrated to JST
                    else:
                        time_jst = 90000 + float(tmp2[GPRMC+1]) # UTC time is transrated to JST
                ...
                if ((float(time_jst)/10000)<=9):
                    ...
                    if (currentDate == 301122):
                        ...
                        currentDate = currentDate + 10000  # difference of date due to time zone is transrated to JST
                else:
                    currentDate = float(tmp2[GPRMC+9])
                #print(currentDate)
                ...
                csv_data = csv_data + '{:0=6}'.format(int(time_jst)) + ',' + tmp2[GPRMC+2] +  ',' + tmp2[GPRMC+3] +  ',' + tmp2[GPRMC+4]
                csv_data = csv_data +  ',' + tmp2[GPRMC+5] +  ',' + tmp2[GPRMC+6] + ','
                csv_data = csv_data +  tmp2[GPRMC+8] +  ',' + '{:0=6}'.format(int(currentDate)) + ','
                if (tmp2[GPRMC+7] == ''):
                    velocity = '0'
                else:
